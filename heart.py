...
heart_df = spark.read.csv(
    "heart_data.csv", schema=heart_schema, header=True
)  # Reading the csv file and inferring the schema

heart_df.printSchema()
print(heart_df.schema)
heart_df.show()  # Showing the data

heart_df.createOrReplaceTempView(
    "HEART_DATA"
...
features_assembler = VectorAssembler(
    inputCols=inputList, outputCol="features"
...
)  # Selecting the features and the heart disease (dependent variable) column

working_df.show(10)

training, test = working_df.randomSplit(
    [0.7, 0.3]
...
lr = LinearRegression(
    featuresCol="features", labelCol="heart_disease"
)  # Creating the model
